...
WITH first_join AS(
 ...
 cohort AS(
  ...
  LEFT JOIN first_join AS f 
			   ON t.customer_id = f.customer_id
 )
SELECT first_join_month,
       month_diff,
       COUNT(DISTINCT customer_id) / FIRST_VALUE(COUNT(DISTINCT(customer_id) OVER (PARTITION BY first_join_month ORDER BY month_diff ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) * 100 AS retention
FROM cohort
